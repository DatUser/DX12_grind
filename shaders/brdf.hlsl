...
static float3 f0 = float3(1.00, 0.71, 0.29); //Pre-computed (default is water value)
static float roughness = 0.2;
static float metalness = 1;// = 0.1;//1 if metallic 0 otherwise

// Use dotNH for microdetails
float3 fresnelSchlick(float dotHV, float3 albedo)
{
  float3 F0 = lerp(f0, albedo, metalness);
  float3 f90 = float3(1.0, 1.0, 1.0);//Pre-computed (here we use water value)
...
float3 cook_torrance_brdf(float3 wo, float3 wi, float3 n, float3 albedo)
{
  ...
  //Represents ks
  float3 nFresnel = fresnelSchlick(dotLH, albedo);
